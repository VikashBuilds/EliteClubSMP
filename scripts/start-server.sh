...
echo "============================================"
echo "  Starting PaperMC Minecraft Server (16GB RAM Ubuntu)"
echo "============================================"
...
if [ ! -f "paper.jar" ]; then
  ...
  # Verify download
  FILE_SIZE=$(stat -c%s paper.jar 2>/dev/null || stat -f%z paper.jar 2>/dev/null || echo "0")
...
# High-performance JVM flags optimized for 16GB RAM Ubuntu & Aikar's Flags
JAVA_FLAGS=(
  -Xms6G
  -Xmx12G
  -XX:+UseG1GC
...
echo "--- Launching Java process with 12GB heap (6GB initial)..."
echo "  The playit.gg plugin will handle TCP tunneling automatically."
echo "  Watch the logs for your server's public address (*.ply.gg)"
echo ""
